...
@Component({
  selector: 'app-enquiry-drop',
  templateUrl: './enquiry-drop.component.html',
  styleUrls: ['./enquiry-drop.component.css']
})
export class EnquiryDropComponent implements OnInit {
  ...
  getReport() {
    ...
    this.report.getEnquiryDropReport(data)
      .subscribe({
        next: (v: any) => {
          this.spinner.hide();
          if (v.success) {
  ...
  generatePdf() {
    ...
    let reportTitle = `Enquiry Drop Report ${this.DatePipe.transform(this.filterForm.value.startDate, 'dd/MM/YYYY')} - ${this.DatePipe.transform(this.filterForm.value.endDate, 'dd/MM/YYYY')}`;
    let filter: any = {};
    ...
    this.dataSource.data.forEach((element: any) => {
      ...
      tempObj.push(i);
      tempObj.push(element?.enquiryAutoId || 'N/A');
      ...
      tempObj.push(element?.studentName || 'N/A');
      tempObj.push(element?.personalContact || 'N/A');
      tempObj.push(this.DatePipe.transform(element?.enquiryDate, "dd/MM/YYYY") || 'N/A');
      tempObj.push(this.DatePipe.transform(element?.dropAt, "dd/MM/YYYY") || 'N/A');
