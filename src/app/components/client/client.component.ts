...
export class ClientComponent implements OnInit {
  ...
  search: string = ""
  displayedColumns: string[] = ['clientAutoId', 'name', 'administrator','phone','email','dob','address','location', 'gstNo', 'companyName', 'createdAt', 'action']

  ClientResponse: Partial<any> = {}
  ...
  getAllClient(event: any) {
    ...
    if (this.totalLoaded == 0 || event == null || ((this.totalLoaded < this.total) && (event.pageIndex > this.pageNo))) {
      this.clientService.getAllClient(data).subscribe({
        next: (result) => {
          this.spinner.hide()
          if (result.success) {
  ...
  openDeleteDialog(id: any) {
    ...
    dialogRef.afterClosed().subscribe((confirmed: boolean) => {
      if (confirmed) {
        this.spinner.show()
        this.clientService.deleteClient({ _id: id }).subscribe({
          next: (v: any) => {
            var result = v
            if (result.success) {
  ...
  openDialog(data: any) {
    ...
    if (data == "") {
      ...
      this.clientForm.patchValue({ _id: undefined })
    }
    else {
      type = 'Update'
    ...
    const dialogRef = this.dialog.open(ClientAddComponent, { disableClose: false, autoFocus: true, width: '40%', data: { type: type, form: this.clientForm } })
    dialogRef.afterClosed().subscribe((confirmed: boolean) => {
      if (confirmed) {
        this.getAllClient(null)
      }
